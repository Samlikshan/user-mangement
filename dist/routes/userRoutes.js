"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
const express_1 = __importDefault(require("express"));
const userController_1 = require("../controllers/userController");
const authMiddleware_1 = require("../middleware/authMiddleware");
const router = express_1.default.Router();
router.get("/", authMiddleware_1.isLoggedIn, userController_1.index);
router.get("/signup", userController_1.getSignUp);
router.post("/signup", userController_1.postSignup);
router.get("/login", userController_1.getLogin);
router.post("/login", userController_1.postLogin);
// router.get("/", index);
router.get("/forgot-password", userController_1.getForgotPassword);
router.post("/forgot-password", userController_1.postForgotPassword);
router.get("/logout", userController_1.logout);
exports.default = router;
